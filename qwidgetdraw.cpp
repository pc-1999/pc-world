...
QWidgetDraw::QWidgetDraw(QWidget *parent) : QWidget(parent)
{
    ...
    pixmap = new QPixmap(this->width()+1000,this->height());//创建与界面等大小的画布
    pixmap->fill(Qt::black);//设置背景色为黑色

    index = 0;//当前水平方向数据更新位置
    ecgDataIndex = 0;
}

void QWidgetDraw::timerEvent(QTimerEvent *event)
{
    drawWaveToPixmap();
    this->update();
}

void QWidgetDraw::drawWaveToPixmap()
{
    QPainter pixPainter;
    pixPainter.begin(this->pixmap);//画到pixmap图片上
...
void QWidgetDraw::paintEvent(QPaintEvent *event)
{
   ...
   // drawSineWave(painter);
    painter->drawPixmap(0,0,*pixmap);//把图片画到控件上
    painter->resetTransform();
    painter->end();
}

void QWidgetDraw::drawEcgWave(QPainter *painter)
{
    QPen pen ;
    pen.setWidth(2);
    pen.setColor(QColor("#00FF57"));
    painter->setPen(pen);
    ...
    painter->setPen(pen);
    painter->drawLine(index,0,index,height());//黑色竖线
    painter->restore();//还原当前配置
    //绘图
    qreal height = this->height();//测量总高度
    QPoint linestart;
    QPoint lineEnd;
    ...
    ecgDataIndex = ecgDataIndex +5;//每五个点画一次
    ...
    lineEnd.setY(y);
    painter->drawLine(linestart,lineEnd);//划线
}
