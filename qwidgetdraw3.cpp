...
QWidgetDraw3::QWidgetDraw3(QWidget *parent) : QWidget(parent)
{
...
void QWidgetDraw3::timerEvent(QTimerEvent *event)
{
    drawWaveToPixmap();
    this->update();
}

void QWidgetDraw3::drawWaveToPixmap()
{
    QPainter pixPainter;
...
void QWidgetDraw3::paintEvent(QPaintEvent *event)
{
...
void QWidgetDraw3::drawRESPWave(QPainter *painter)
{
    QPen pen ;
    pen.setWidth(2);
    pen.setColor(QColor("#FF8D00"));
    painter->setPen(pen);
    ...
    qreal value = ecgWave3[ecgDataIndex];
    int y = round(height/2-((value-15)/150)*height/2);
    qDebug()<<y;
    linestart.setY(y);
    ...
    value = ecgWave3[ecgDataIndex];
     y = round(height/2-((value-15)/150)*height/2);
    lineEnd.setY(y);
